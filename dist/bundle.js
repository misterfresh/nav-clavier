    ...
    for (var key in methods) {
      {
        if (typeof methods[key] !== 'function') {
          warn("Method \"".concat(key, "\" has type \"").concat(_typeof(methods[key]), "\" in the component definition. ") + "Did you reference the function correctly?", vm);
        }
  ...
  var AppNavigation = {
    ...
    methods: {
      navigate: function navigate(event) {
        ...
        var updatedCoords = _toConsumableArray(this.navCoords);
        ...
        switch (direction) {
          ...
          case 'RIGHT':
            {
              if (this.navCoords[1] === this.navTree[this.navCoords[0]].length - 1) {
                updatedCoords[1] = 0;
              } else {
                updatedCoords[1]++;
              }
      ...
      getDirectionFromEvent: function getDirectionFromEvent(event) {
        var code = event.code;
